...
\begin{abstract}
Troups is a novel transaction manager for cloud datastores of the BigTable family that enables full ACID transactions at large scale for applications that exhibit transaction locality. It extends the BigTable programming model with the ability to co-locate related rows and implements efficient local transactions across such row groups through an observer-based protocol. Troups also aids users in selecting row groups by providing an algorithm that analyzes transaction logs to identify tables or rows frequently accessed in the same transactions. We also introduce the idea of swapping data items by observing their cyclic frequencies in the past. Troups improves the consistency guarantees of BigTable datastores without significantly impacting their high scalability or availability.
\end{abstract}
...
\section{Evaluation}
We evaluated Troups by comparing its response time with that of HBase on different cluster sizes to show the overhead imposed by transactions. The workload used in all cases consist of random read-write single-group transactions. Evaluation was done on cluster sizes of 4 and 8 nodes running one Hadoop/HBase master and 3-7 data nodes.  Each node in the cluster was configured from the same image using Amazon EC2 as a high CPU instance.  Two Troups configurations were evaluated to show the difference of the timestamp service implementations.

The measurements show that transactions increase the respone time mainly due to additional remote procedure calls and log operations. Depending on the setup of Troups, the transaction overhead for these small read-write transactions is between 3x and 6x. However the ability to scale horizontally across multiple nodes is preserved. The results of the evaluation are shown in Figure ~\ref{tp}. 
...
We also measured the effect of increasing the number of clients in a fixed-sized cluster of 3 nodes. While the response time and throughput of Troups is higher and lower respectively when compared to HBase, the shapes of the curves grow at similar rates. In fact, Troups scales sligtly better in that the response time about doubles as the number of clients is increased to 16, wheras Hbase's response time triples. Also, Troup's throughput increases by a factor of almost 7 whereas HBase's throughput increases by a factor of almost 6.

\begin{figure}[h]
\centering
\hspace*{-.2in}
\includegraphics[scale=0.7]{images/eval1.pdf}
\caption{Response time as number of clients increase}
\label{tp}
\end{figure}
...
\centering
\hspace*{-.2in}
\includegraphics[scale=0.7]{images/eval2.pdf}
\caption{Transactions per second as number of clients increase}
\label{tp}
\end{figure}
...
There are several ways to improve the current transaction manager. First, our evaluation has shown that the table-based log implementation presents a bottleneck both in terms of latency and throughput. Alternative implementation strategies should be explored. Furthermore, other concurrency control protocols could be evaluated against the current OMVTO implementation. Snapshot isolation or multiversion two-phase locking are plausible candidates. It would also be interesting to add support for range queries and measure the cost of partial key scans. Finally, different recovery and garbage collection strategies could be compared. In particular, group commit optimizations as described in ~\cite{Weikum:2001:TIS} could be explored.

Group detection and formation can also be enhanced in several ways. Static row grouping can be defined as an optimization problem that balances the cost of distributed transactions against the cost of moving data and using partial key scans. If row grouping is made transparent to users through a virtual table layer, online grouping algorithms could be explored that dynamically group data at runtime. Such algorithms would be particularly well suited for applications with frequently changing access patterns. It could also be investigated for what degrees of locality fully generalized ACID transactions are feasible.

\section{Conclusion}
We have described a transaction programming model and implementation for BigTable. Our initial results show that it improves consistency guarantees without significantly impacting scalability. Our approach requires no changes to BigTable and can be deployed without additional hardware or processes alongside non-transactional APIs in the same cluster.

We have also presented a flexible mechanism for forming static or dynamic row groups. We use this mechanism to co-locate rows in order to execute local transactions across them, but it is useful in many other cases. For example, to place data groups closer to users to improve performance, or to form groups in a way that better balances server load.
...
Moreover, we explored the use of our time swapping algorithm to partition data right before a user needs to access it to improve real time performance. We implemented our algorithm and simulated a test run.
